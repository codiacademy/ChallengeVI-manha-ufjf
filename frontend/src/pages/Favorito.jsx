...
const Favorito = () => {
  ...
  useEffect(() => {
    const fetchFavoriteItems = async () => {
      try {
        ...
        setError("Erro ao carregar os favoritos. Tente novamente mais tarde.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRemoveFromFavorites = async (id) => {
    try {
      ...
      setError("Erro ao remover o item dos favoritos.");
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center bg-gradient-to-b from-violet-100 to-violet-300 text-black min-h-screen">
        <h1 className="text-4xl font-bold mt-8 mb-6 tracking-wide">FAVORITOS</h1>
        ...
        <section className="w-full max-w-5xl mt-8 bg-gradient-to-r from-purple-600 to-violet-400 rounded-lg shadow-lg p-6">
          ...
          {itensFavoritos.length > 0 ? (
            ...
            <p className="text-center text-lg">Nenhum item nos favoritos ainda.</p>
          )}
        </section>
